...
- (void) dismissChatUI {
    /// save last message to the disk whenever the chat is open.
    ZDKChatLog *lastLog = ZDKChat.instance.chatProvider.chatState.logs.lastObject;
    if (lastLog.id != nil ) {
        [NSUserDefaults.standardUserDefaults setValue:lastLog.id forKey:@"ZendeskLastLogId"];
        [NSUserDefaults.standardUserDefaults synchronize];
    }
    /// push a notification that informs about chat's UI got dismissed.
    [NSNotificationCenter.defaultCenter postNotificationName:@"ZendeskChatUpdate" object:nil];
